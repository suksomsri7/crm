...
2. **Flow ที่ต้องทำ**
   ...
   - เรียก GET {BASE_URL}/api/entities/check?externalId=CUST-001&brandId={brandId}
   - ถ้า exists=false → สร้างใหม่ด้วย POST {BASE_URL}/api/leads หรือ POST {BASE_URL}/api/customers (ส่ง externalId ด้วย)
   - ถ้า exists=true และ inLeads → อัปเดตด้วย PUT {BASE_URL}/api/leads/{lead.id}
   - ถ้า exists=true และ inCustomers → อัปเดตด้วย PUT {BASE_URL}/api/customers/{customer.id}
...
กฎ:
1. เรียก GET {BASE_URL}/api/entities/check?externalId={id}&brandId={id} ก่อนเพิ่มข้อมูล (กันซ้ำ)
2. exists=false → POST /api/leads หรือ /api/customers (ส่ง externalId)
3. exists=true → PUT /api/leads/{id} หรือ /api/customers/{id} เพื่ออัปเดต
4. Login ก่อน: POST {BASE_URL}/api/auth/callback/credentials (username, password) แล้วใช้ credentials: 'include'
...
| ตรวจสอบซ้ำ (ต้องเรียกก่อนเพิ่ม) | GET | /api/entities/check?externalId=xxx&brandId=xxx |
| สร้าง Lead | POST | /api/leads |
| แก้ไข Lead | PUT | /api/leads/{id} |
| สร้าง Customer | POST | /api/customers |
| แก้ไข Customer | PUT | /api/customers/{id} |
| บันทึก Chat Log (Lead) | POST | /api/leads/{id}/chat-logs |
| บันทึก Chat Log (Customer) | POST | /api/customers/{id}/chat-logs |
| รายการ Sources | GET | /api/sources?active=true |
...
|--------|----------|----------|
| CRM_BASE_URL | URL ของ CRM | https://crm.yourdomain.com |
...
```
1. Login
POST {CRM_BASE_URL}/api/auth/callback/credentials
Content-Type: application/x-www-form-urlencoded
username=superadmin&password=@Superadmin252322

2. ตรวจสอบซ้ำ
GET {CRM_BASE_URL}/api/entities/check?externalId=CUST-001&brandId={BRAND_ID}
Cookie: (จาก step 1)
...
PUT {CRM_BASE_URL}/api/leads/{lead.id}
Content-Type: application/json
Cookie: (จาก step 1)
{
  "phone": "0899999999",
...
5. บันทึก Chat Log
POST {CRM_BASE_URL}/api/leads/{lead.id}/chat-logs
Content-Type: application/json
Cookie: (จาก step 1)
{
  "channel": "LINE",
...
# 2. ตรวจสอบซ้ำ
curl -b cookie.txt "http://localhost:3000/api/entities/check?externalId=CUST-001&brandId=YOUR_BRAND_ID"

# 3. สร้าง Lead
curl -b cookie.txt -X POST "http://localhost:3000/api/leads" \
